...
class MyModel extends CI_Model {
            ...
            if (hash_equals($hashed_password,md5($password))) {
              ...
              $this->db->insert('users_authentication',array('users_id' => $id,'token' => $token,'expired_at' => $expired_at));  
               if ($this->db->trans_status() === FALSE){
                  ...
                  $result = array('status' => 200,'message' => 'Successfully login.','id' => $id, 'token' => $token);
    ...
    public function signup($username,$password)
    {
                ...
                $q  = $this->db->select('password,username')->from('users')->where('username',$username)->get()->row();
                $db_username = $q->username;

               
       
        if(!$db_username){
            $sifrelenecek = md5($password);
                $this->db->insert('users',array('username' => $username,'password' => $sifrelenecek,'name' => 'name'));
               ...
               $expired_at = date("Y-m-d H:i:s", strtotime('+12 hours'));
                ...
                $y  = $this->db->select('id')->from('users')->where('username',$username)->get()->row();
           
                $db_id = $y->id;
                
               $this->db->trans_start();
               $this->db->where('id',$db_id)->update('users',array('last_login' => $last_login));
               $this->db->insert('users_authentication',array('users_id' => $db_id,'token' => $token,'expired_at' => $expired_at));
               if ($this->db->trans_status() === FALSE){
                  $this->db->trans_rollback();
                  return array('status' => 500,'message' => 'Internal server error.');
               } else {
                  $this->db->trans_commit();
                   return array('status' => 200,'message' => 'Successfully login.','id' => $db_id, 'token' => $token);
               }
        }else if(hash_equals($username,$db_username)){
            return array('status' => 204,'message' => 'Username already exist.');
            //echo "Username already exist.";
        } 
        else{
            return array('status' => 205,'message' => 'Username already exist.');
           // echo "Username already exist.";
        }
